	...
	for (int i = 0; i < SpeedMult; i++) {
        ...
        for(int j = 0 ; j < std::get<2>(current) ; j++){
            ...
            switch (std::get<0>(current)) {
                ...
                //owo
                case 4:
                    EnemyGroup->AddNewObject(enemy = new OtakuEnemy(SpawnCoordinate.x, SpawnCoordinate.y));
					break;
				default:
					continue;
			}
...
void PlayScene::OnKeyDown(int keyCode) {
	...
	if (keyCode==ALLEGRO_KEY_TAB) {
        ...
        bool flag = true;
        //for (list<int>::iterator it = keyStrokes.begin(); it != keyStrokes.end(); ++it)
        //owo
        int ooo=0;
        for(auto c: keyStrokes){
            //printf("code[%d] = %d",ooo,code[ooo]);
            if(c!= code[ooo]) {
                flag = false;
                break;
            }
...
void PlayScene::ConstructUI() {
    ...
    //Button 3
    btn = new TurretButton("play/floor.png", "play/dirt.png",
        Engine::Sprite("play/turret-4.png", 420, BlockSize*MapHeight, 0, 0, 0, 0)
        , 410, 128*MapHeight, TCellTurret::Price);
...
void PlayScene::UIBtnClicked(int id) {
	...
	//owo
    else if(id==2&&money >=TCellTurret::Price)
