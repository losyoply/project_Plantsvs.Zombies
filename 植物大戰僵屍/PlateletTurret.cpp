...
PlateletTurret::PlateletTurret(float x, float y) :
	Turret("play/turret-2.png", x, y, Price, 0.5, 450) {
...
void PlateletTurret::CreateBullet() {
	...
	//by me
	getPlayScene()->EffectGroup->AddNewObject(new ShootEffect(Position.x+50, Position.y+30));
...
void PlateletTurret::Update(float deltaTime){
	...
	if (Target) {
		...
		if (std::abs(radian) <= maxRotateRadian)
			...
			rotation = ((std::abs(radian) - maxRotateRadian) * originRotation + maxRotateRadian * targetRotation) / radian;
