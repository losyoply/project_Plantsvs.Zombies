	...
	for (int i = 0; i < SpeedMult; i++) {
        ...
        for(int j = 0 ; j < std::get<2>(current) ; j++){
            ...
            switch (std::get<0>(current)) {
                ...
                //
                case 4:
                    EnemyGroup->AddNewObject(enemy = new taikoenemy(SpawnCoordinate.x, SpawnCoordinate.y));
					break;
				default:
					continue;
			}
...
void PlayScene::OnMouseUp(int button, int mx, int my) {
	...
	const int y = my / BlockSize;
	if (button & 1) {
        if (mapState[y][x] == TILE_OCCUPIED && flag==1)
        {
...
void PlayScene::OnKeyDown(int keyCode) {
	...
	if (keyCode==ALLEGRO_KEY_TAB) {
        ...
        for(auto c: keyStrokes){
            if(c!= code[countofcheat]) {
                check = false;
                break;
            }
            countofcheat++;
        }
        if(check&&DebugMode&&countofcheat==7){
            this->money += 10000;
            this->lives += 10;
	...
	// TODOTODO 2 (5/8): Make the E key to create the 3th turret.
	//by me
...
void PlayScene::ConstructUI() {
    ...
    // Button 3
    btn = new TurretButton("play/floor.png", "play/dirt.png",
        Engine::Sprite("play/kirby.png", 420, BlockSize*MapHeight, 0, 0, 0, 0)
        , 410, 128*MapHeight, KirbyTurret::Price);
    btn->SetOnClickCallback(std::bind(&PlayScene::UIBtnClicked, this, 2));
    UIGroup->AddNewControlObject(btn);
    //BUTTON 4
    btn = new TurretButton("play/floor.png", "play/dirt.png",
		Engine::Sprite("play/turret-3.png", 540, BlockSize*MapHeight, 0, 0, 0, 0)
		, 530, 128*MapHeight, NoAtkTurret::Price);
    btn->SetOnClickCallback(std::bind(&PlayScene::UIBtnClicked, this, 3));
	UIGroup->AddNewControlObject(btn);
    //BUTTON 5
	btn = new TurretButton("play/floor.png", "play/dirt.png",
		Engine::Sprite("play/turret-4.png", 660, BlockSize*MapHeight, 0, 0, 0, 0)
		, 650, 128*MapHeight, CircleTurret::Price);
	...
	//BTN 7
    btn = new TurretButton("play/floor.png", "play/dirt.png",
		Engine::Sprite("play/heal.png", 900, BlockSize*MapHeight, 0, 0, 0, 0)
		, 890, 128*MapHeight, 0);
...
void PlayScene::UIBtnClicked(int id) {
	...
	//by me
    else if(id == 2&&money >=KirbyTurret::Price)
        preview = new KirbyTurret(0, 0);
    else if(id==3&&money>=NoAtkTurret::Price)
        preview = new NoAtkTurret(0,0);
    else if(id==4&&money>=CircleTurret::Price)
    ...
    else if(id==6&&money>=HealTurret::Price)
    {
         preview = new HealTurret(0,0);
         flag = 1;
    }
...
void PlayScene::TurnTileFloor(int y, int x)
{
    mapState[y][x] =TILE_FLOOR;
}
